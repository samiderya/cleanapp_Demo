...
{
    public class BackgroundRepository : IBackgroundRepository, IDisposable
    {
        private CleanerDBContext _context;

        public BackgroundRepository(CleanerDBContext context)
        {
            _context = context ?? throw new ArgumentNullException(nameof(context));

        }

        public async Task<bool> Save(Background_check model)
        {
            try
            {
                await _context.Background_check.AddAsync(model);
